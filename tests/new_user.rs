...
fn fails_with_nonexistent_admin_key_path() {
    let root = setup::setup_init_cd();
    let admin_key_path = PathBuf::from("nonexistent");
...
fn fails_with_invalid_admin_key_format() {
    let root = setup::setup_init_cd();
    let admin_key_path = common::make_key_file(&root.path(), "admin.pem", TestKey::AdminKeyPub);
...
fn fails_with_admin_no_privileges() {
    ...
    let _ = match proton_cli::new_user(&admin_key_path, &user_key_path, "userA") {
        Ok(u) => u,
