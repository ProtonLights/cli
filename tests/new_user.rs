...
fn works_with_new_and_existing_protonfile() {
    ...
    try_new_user(root.path(), "Test User", "a.pub", TestKey::GoodKeyPub);
    try_new_user(root.path(), "Test User 2", "b.pub", TestKey::GoodKey2Pub);
}
...
fn fails_with_a_nonexistent_protonfile() {
    ...
    try_new_user(root_dir.path(), "Username", "a.pub", TestKey::GoodKeyPub);
}
...
fn fails_with_non_pem_key() {
    let root = common::setup_init_cd();
    try_new_user(root.path(), "Test User", "bad_pub_key.pub", TestKey::BadPubKeyPub);
}
...
fn fails_with_duplicate_user_key() {
    ...
    try_new_user(root.path(), "Test User 1", "a.pub", TestKey::GoodKeyPub);
    try_new_user(root.path(), "Test User 2", "b.pub", TestKey::GoodKeyPub);
}
...
fn fails_with_duplicate_user_name() {
    ...
    try_new_user(root.path(), "Test User", "a.pub", TestKey::GoodKeyPub);
    try_new_user(root.path(), "Test User", "b.pub", TestKey::GoodKey2Pub);
}
